...
class HttpReq(object):
    ...
    def _build_header(self, custom_headers=None):
        # ua = random.choice(USER_AGENT_LIST)
        ua = "SECSOSO(https://secsoso.com)"
        headers = {
    ...
    async def request(self, url, method="get", params=None, custom_headers=None, auth="", retry_limit=0):
        await asyncio.sleep(random.uniform(0.2, 0.8))
        headers = self._build_header(custom_headers)
        return await self.retry_request(url, method, params, headers, auth, retry_limit)
    ...
    async def retry_request(
        ...
        if retry_times <= retry_limit:
            ...
            try:
                content, resp = await _request_method(url, params, headers, auth)  # type: ignore
            except (
    ...
    async def _get(self, url, params=None, headers=None, auth: Optional[aiohttp.BasicAuth] = None):
        if headers is None:
            headers = {}
        async with aiohttp.ClientSession(connector=self._get_connector(), auth=auth) as session:
            async with session.get(url, params=params, headers=headers, ssl=False) as resp:
                content = await resp.text(encoding="utf-8")
        return content, resp

    async def _post(self, url, data=None, headers=None, auth: Optional[aiohttp.BasicAuth] = None):
        if headers is None:
            headers = {}
        async with aiohttp.ClientSession(connector=self._get_connector(), auth=auth) as session:
